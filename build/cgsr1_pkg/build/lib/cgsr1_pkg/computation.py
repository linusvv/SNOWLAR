    ...
    def publish_winch(self):
        ...
        with manual_control_lock:
            ...
            if abs(self.chainLeft - self.chainRight) <= 0.1: ##for now, chainLeft and chain Right should be parallel
                winch_msg.linear.x = self.translation_Factor*(math.sin(self.angle)* self.chainLeft)
                winch_msg.linear.y = -1*self.translation_Factor*(math.sin(self.angle)* self.chainLeft)
            else:
